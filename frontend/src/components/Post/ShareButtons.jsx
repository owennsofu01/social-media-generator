...
const ShareButtons = ({ post, onNotification }) => {
  ...
  const baseButtonClass =
    "p-2 rounded-full transition duration-150 text-gray-500 hover:text-white shadow-sm";
    ...
    switch (platform) {
      ...
      case "threads":
        navigator.clipboard.writeText(post.title)
          ...
          .catch(() => onNotification?.(`❌ Failed to copy content for ${platform}.`, "error"));
        break;
      default:
    ...
    <div className="flex gap-2">
      {platforms.map((p) => (
        <button
          ...
          title={p.title}
        >
          {React.cloneElement(p.icon, { size: iconSize })}
        </button>
      ))}
    </div>
  );
